...
namespace Antistress
{
    public class Popit2D : MonoBehaviour
    {
        ...
        {
            if (Popit2DManager.Epopping)
                Popit2DManager.anitstressPopit2dManagerInstance.OnEpopReset += HomeBtnReset;
        ...
        private void OnDisable()
        {
            ...
            for (int i = 0; i < popits.Length; i++)
            {
                if (popits[i].transform.GetComponentInChildren<PopSounds>())
        ...
        {
            Popit2DManager.anitstressPopit2dManagerInstance.fillAmountImage.fillAmount = 0;
            Popit2DManager.anitstressPopit2dManagerInstance.value = 1f / popits.Length;
            if (initialized)
            {
                ...
                for (int i = 0; i < popits.Length; i++)
                {
                    //print("//////////////" + i + "/////////////////////////");    // number of popits that a toy have
        ...
        void MobileInput()
        {
            if (Input.touchCount > 0)
            {
                ...
                {
                    if (Popit2DManager.Epopping == false)
                        CheckHit(touch.position);
                    if (Popit2DManager.Epopping == true)
        ...
        void CheckHit(Vector3 position)
        {
            ...
            if (hit.collider != null && hit.collider.CompareTag("obj"))
            {
                ...
                if (Popit2DManager.anitstressPopit2dManagerInstance.fillAmountImage.fillAmount < 1)
                {
                    Popit2DManager.anitstressPopit2dManagerInstance.FilImageAmountCall();
                }
                count++;
                int p = Random.Range(1 , 6);
                SoundManager.instance.PlayEffect_Instance(21);//anitstresss
        ...
        void Check_E_Hit(Vector3 pos)
        {
            ...
            if (hit.collider != null && hit.collider.CompareTag("obj"))
            {
                ...
                {
                   //11 MoreMountains.NiceVibrations.MMVibrationManager.Haptic(MoreMountains.NiceVibrations.HapticTypes.RigidImpact);
                    Debug.Log("in antistress game vibrations are " + Dino.SoundManager.isHaptic);
                }
        ...
        {
            //print("Count     :" + count + " List Size   :" + epops.Count);   // count of number of popits on a toy
            if (count >= epops.Count)
            {
                ...
                Popit2DManager.anitstressPopit2dManagerInstance.PlayParticles();
                SoundManager.instance.PlayEffect_Instance(1);
                Invoke("ShowInterstitailAD", 1f);

                if (Popit2DManager.Adcounter > 2)
                {
                    Popit2DManager.Adcounter = 0;
        ...
        public void HomeBtnReset()
        {
            ...
            Maxnum = 0;
            count = 0;
            for (int i = 0; i < Popit2DManager.anitstressPopit2dManagerInstance.Levels.Length; i++)
            {
                Popit2DManager.anitstressPopit2dManagerInstance.Levels[i].SetActive(false);
            }

            Popit2DManager.anitstressPopit2dManagerInstance.EpopsSelectionUI.SetActive(true);
            //epops.Clear();
        ...
        void ResetPop()
        {
            Popit2DManager.Adcounter++;
            EnablePopit();
            GenerateRandomList();
            if (Popit2DManager.Adcounter > 3)
            {
                Popit2DManager.Adcounter = 0;
                // GoogleAdMobController.THIS.ShowR1InterstitialAd(); //AdCallPosition
            }
        ...
        void ChecktAllPophit()
        {
            // if(transform.parent.gameObject!=null)
            {
                ...
                if (counter >= childCount)
                {
                    ...
                    spikes.Play();
                    Popit2DManager.anitstressPopit2dManagerInstance.FileUserExpericenBar(3.6f , transform.gameObject.name);
                    SoundManager.instance.PlayEffect_Instance(1);
