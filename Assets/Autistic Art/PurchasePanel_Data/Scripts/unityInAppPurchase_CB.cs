...
public class unityInAppPurchase_CB : MonoBehaviour, IStoreListener
{
...
#pragma warning restore 0414
    ...
    private static bool m_PurchaseInProgress = false;
    public bool IsInitilized = false;

    public static unityInAppPurchase_CB instance;

    public delegate void OnFailed();

    public static event OnFailed Failed;

    static int inAppValue = 0;
    ...
    [Header("Unity InApp Define")] public InAppKeys[] InAppIds;
    public static InAppKeys[] keys;
    public GameObject inAppGameObject;
    public static GameObject inappObj;
    [HideInInspector] public unityInAppPurchase_CB IAP;
    ...
    public void OnInitialized(IStoreController controller, IExtensionProvider extensions)
    {
        ...
        foreach (var item in controller.products.all)
        {
            if (item.availableToPurchase)
            {
                ...
                {
                    if (item.definition.id == "remove_ads" && item.definition.type == ProductType.NonConsumable)
                    {
    ...
    public static void AfterPurchased(int value)
    {
        try
        {
            ...
            if (keys[value].purchaseType == PurchaseType.Remove_Ads)
            {
                ...
                try
                {
                    //InitializeFirebase_CB._Instance.LogFirebaseEvent(SceneManager.GetActiveScene().name + "_Loaded");
                }
                catch (Exception ex)
                {
                ...
                //if (!IntitializeAdmob.instance.CehckMemory())
                //{
                ...
                //IntitializeAdmob.instance.DestroyBannerView();
            ...
            else if (keys[value].purchaseType == PurchaseType.Unlock_All)
            {
                ...
                PlayerPrefs.SetInt("Purchased", 1);
                InitializeFirebase_CB.instance.LogFirebaseEvent("Inapp_no_" + value + "_Bought");
                SceneManager.LoadScene("Selection");
            ...
            {
                PlayerPrefs.SetInt(keys[value].Id, 1);
                InitializeFirebase_CB.instance.LogFirebaseEvent(keys[value].SceneName + "_Bought");
                SceneManager.LoadScene(keys[value].SceneName);
            }
    ...
    public PurchaseProcessingResult ProcessPurchase(PurchaseEventArgs e)
    {
        ...
        if (IsRestoring)
        {
            productIdsForRestore.Add(e.purchasedProduct.definition.id);
        }
    ...
    public void Awake()
    {
        ...
        StartCoroutine(InitilizeIAP());
    }

    IEnumerator InitilizeIAP()
    {
        ...
        {
            builder.AddProduct(keys[i].Id, keys[i].productType, new IDs
            {
            });
        }
        ...
        //// B) unityInAppPurchase_CB (this) displays the "Restore" GUI button for Samsung Galaxy Apps
    ...
    public Product GetProductDetail(string productId)
    {
        ...
        if (IsInitilized)
        {
            p = m_Controller.products.WithID(productId);
        }
    ...
    private void OnTransactionsRestored(bool success)
    {
        ...
        if (success)
        {
            for (int i = 0; i < productIdsForRestore.Count; i++)
            {
                ...
                {
                    if (keys[j].Id == productIdsForRestore[i])
                    {
                        if (keys[j].productType == ProductType.NonConsumable
                            || keys[j].productType == ProductType.Subscription)
                        {
                            AfterPurchased(j);
                        }
            ...
            {
                PurchaseMsg.instance.ShowToast();
            }
    ...
    void PurchaseButtonClick(string productID)
    {
        ...
        {
            Debug.Log("No product has id " + productID);
            return;
        }
